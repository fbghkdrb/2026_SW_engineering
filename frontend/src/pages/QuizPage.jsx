    ...
    return (
      <div className="min-h-screen bg-wt-sky pb-20">
        ...
        <main className="max-w-2xl mx-auto px-4 pt-5 pb-4 space-y-5">
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5">
            ...
            {loadingDays ? (
              ...
              <div className="text-center space-y-3 py-4">
                ...
                <button onClick={loadDays} className="text-sm text-wt-orange underline">
                  다시 시도
                </button>
              </div>
            ) : days.length === 0 ? (
              ...
              return (
                <motion.button
                  ...
                  className={`w-full text-left px-5 py-4 rounded-2xl border-2 transition-all ${
                    isSelected
                      ? "border-wt-orange bg-wt-orange-light"
                      : "border-gray-100 bg-white hover:border-wt-orange"
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <span className="text-xl">{t.icon}</span>
                    <div>
                      <p className={`font-bold text-sm ${isSelected ? "text-wt-orange" : "text-gray-800"}`}>
                        {t.label}
                      </p>
                      <p className="text-xs text-gray-400 mt-0.5">{t.desc}</p>
                    </div>
                    {isSelected && (
                      <span className="ml-auto text-wt-orange font-bold text-lg">✓</span>
                    )}
                  </div>
                </motion.button>
              );
          ...
          {/* 시간 제한 토글 카드 */}
          <div
            className={`bg-white rounded-2xl border-2 transition-all px-5 py-4 cursor-pointer ${
              timedMode ? "border-wt-orange" : "border-gray-100"
            }`}
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className={`font-bold text-sm ${timedMode ? "text-wt-orange" : "text-gray-800"}`}>
                  시간 제한 모드
                </p>
                <p className="text-xs text-gray-400 mt-0.5">
                  문제당 {TIMER_SECONDS}초 제한 · 초과 시 자동 오답 처리
                </p>
              </div>
              {/* 토글 스위치 */}
              <div
                className={`relative w-11 h-6 rounded-full transition-colors ${
                  timedMode ? "bg-wt-orange" : "bg-gray-200"
                }`}
              >
                <div
                  className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-transform ${
                    timedMode ? "translate-x-5" : "translate-x-1"
                  }`}
                />
              </div>
            </div>
          ...
          >
            {selectedDay
              ? `Day ${selectedDay} · ${QUIZ_TYPES.find((t) => t.id === quizType)?.label} 시작 →`
              : "Day를 선택해주세요"}
          </motion.button>
        </main>
        ...
        <div className="max-w-2xl mx-auto flex items-center justify-between px-5 py-3">
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-gray-400">
              {QUIZ_TYPES.find((t) => t.id === quizType)?.label}
            </p>
          </div>
      ...
      <main className="px-4 py-5 pb-8 max-w-2xl mx-auto w-full space-y-4">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {quizType === "SHORT" && (
              <>
                <p className="text-xs text-wt-orange font-bold uppercase tracking-widest mb-5">
                  한국어 뜻 → 영어 단어
                </p>
            ...
            {quizType === "BLANK" && (
              <>
                ...
                <div className="inline-block bg-wt-orange-light border border-wt-sky-dark rounded-xl px-4 py-1.5">
                  <p className="text-sm font-semibold text-wt-orange">
        ...
        {isMultiple && (
          <div className="space-y-2.5">
            {currentQuestion.choices.map((choice, i) => {
              ...
              if (feedback) {
                if (isCorrectChoice) {
                  cls += "border-green-400 bg-green-50 text-green-800 rounded-2xl";
                } else if (isSelected) {
                  cls += "border-red-400 bg-red-50 text-red-700 rounded-2xl";
                } else {
                  cls += "border-gray-100 bg-white text-gray-400 rounded-2xl";
                }
              } else {
                cls +=
                  "border-gray-200 bg-white text-gray-700 hover:border-wt-orange hover:bg-wt-orange-light rounded-2xl cursor-pointer";
              }
        ...
        {!isMultiple && (
          <motion.div
            animate={
              feedback && !feedback.isCorrect ? { x: [0, -8, 8, -8, 8, 0] } : {}
            }
            transition={{ duration: 0.38 }}
          >
            <input
          ...
          {feedback && (
            ...
            >
              {feedback.isCorrect ? (
                ...
                <span>
                  ✗ 오답&nbsp;·&nbsp;정답:{" "}
                  <span className="font-extrabold">{feedback.correctAnswer}</span>
                </span>
              )}
            </motion.div>
          )}
