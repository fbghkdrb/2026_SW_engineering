...
sqlCREATE DATABASE wordtama;

2. 백엔드 설정 및 실행
application.properties 설정
src/main/resources/application.properties 파일에서 DB 정보를 본인 환경에 맞게 수정합니다.
propertiesspring.datasource.url=jdbc:mysql://localhost:3306/wordtama
...
MySQL에서 wordtama 데이터베이스 생성
application.properties에 DB 계정 정보 입력
백엔드 실행 (./gradlew bootRun)
프론트엔드 실행 (npm install → npm start)
